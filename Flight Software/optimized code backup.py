...
log_delay = 6               # Waits this many seconds before the logging starts. Delay so the rocket can be set up to launch before logging starts
...
if development_mode == 0:
    while True:
        if bmp.altitude >= STARTING_ALTITUDE + 3 and logged_liftoff == 0:
            with open("/sd/" + file_name, "a") as f:
...
while True:    
    ...
    with open("/sd/" + file_name, "a") as f:    
        ...
        # Arm
        if chute_armed == 0 and bmp.altitude >= STARTING_ALTITUDE + 50: 
        ...
        # Deploy - close relay
        if chute_armed == 1 and bmp.altitude <= STARTING_ALTITUDE + 49: # once the rocket sinks below 50 meters it fires the chute\
            DATA_CYCLES_CHUTE = data_cycles 
            chute_relay.value = True
            print("Parachute relay on")

            if logged_chute_deploy == 0:    # try putting this in the above if statement
            ...
            # Close relay
            if logged_chute_deploy == 1 and DATA_CYCLES_CHUTE >= 10:   # waits 10 data cyles before opening the relay 
                chute_relay.value = False   
    ...
    if data_cycles > 50 and bmp.altitude <= STARTING_ALTITUDE + 5:  # so logging doesnt stop on the pad
        with open("/sd/" + file_name, "a") as f:   
            f.write(event_comma_count + "Stopped logging; low altitude met. (" + str(bmp.altitude) + "m)\n")
            break

    if logged_chute_deploy == 1 and data_cycles >= log_stop_count:   # backup to the code above
        with open("/sd/" + file_name, "a") as f:   
