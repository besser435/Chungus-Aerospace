"""https://github.com/DougManuel/CO2-logger
We borrowed their code because it's simmilar to what we need to
do and we dont know how to do it. And by borrowed I mean ripped off.
"""
...
import busio
import digitalio
import adafruit_bmp3xx #Temperature, pressure and altimeter sensor
...
bmp388 = adafruit_bmp3xx.Adafruit_BMP3xx_I2C(i2c) # 0x77
bmp388.sea_level_pressure = 1008 # calibrate the pressure
...
filename = "flight_log" + f"{t.tm_year}-{t.tm_mon:02d}-{t.tm_mday:02d}_{t.tm_hour:02d}-{t.tm_min:02d}" + ".csv"
...
while True:
    ...
    if bmp388.data_available:
        ...
        with open("/sd/" + filename, "a") as f:
            ...
            f.write(f"{bmp388.altitude:1f},{bmp388.pressure:1f},{bmp388.temperature:1f}\n")
        ...
        print("Altitude: %0.1f meters" % bmp388.altitude)
        print("Pressure: %0.1f hPa" % bmp388.pressure)
        print("Temperature: %0.1f C" % bmp388.temperature)
        print("")
